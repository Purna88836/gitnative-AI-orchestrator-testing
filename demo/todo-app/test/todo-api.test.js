...
async function createApiHarness() {
  ...
  const publicDir = join(tempDirectory, 'public');

  await mkdir(publicDir, { recursive: true });
  await writeFile(join(publicDir, 'index.html'), '<!DOCTYPE html><title>Todo Demo</title>', 'utf8');
...
test('todo API serves the browser entrypoint', async () => {
...
test('todo API supports create, list, update, and delete flows', async () => {
  ...
  try {
    ...
    assert.ok(createResult.body.todo.updatedAt);
